...
const MinimalCV: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-100 to-gray-200 p-6 rounded-2xl shadow-2xl border-4 border-gray-700 max-w-3xl mx-auto w-full font-sans">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-gray-700 text-lg">Professional Minimal</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-gray-50 p-4 rounded-lg text-center border-2 border-gray-400">
          <div className="text-2xl font-bold text-gray-900">{totalEducation}</div>
          <div className="text-sm text-gray-700">Education</div>
        </div>
        <div className="bg-gray-50 p-4 rounded-lg text-center border-2 border-gray-400">
          <div className="text-2xl font-bold text-gray-900">{data.experience.length}</div>
          <div className="text-sm text-gray-700">Positions</div>
        </div>
      </div>
          ...
          {data.experience.map((exp, i) => (
            <div key={i} className="bg-white border-2 border-gray-400 rounded-lg p-4 flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <span className="w-8 h-8 bg-gray-500 rounded-full flex items-center justify-center text-white font-bold">{i + 1}</span>
                <span className="font-bold text-gray-900 truncate max-w-[120px] min-w-0">{exp.position}</span>
                <span className="text-gray-700 text-sm">@ {exp.company}</span>
              </div>
              <div className="text-xs text-gray-600">{exp.startDate.toLocaleDateString()} - {exp.endDate ? exp.endDate.toLocaleDateString() : 'Present'}</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {data.skills.map((skill, i) => (
            <div key={i} className="flex items-center gap-2 min-w-0 bg-gray-50 border border-gray-300 rounded p-2">
              <span className="text-gray-800 font-semibold truncate max-w-[100px] min-w-0">{skill.name}</span>
              <div className="flex-1 min-w-0 w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                <div className="h-full bg-gray-500 rounded-full" style={{ width: `${(skill.level / 10) * 100}%` }} />
              </div>
              <span className="text-xs text-gray-700 w-6 text-right">{skill.level}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Education Path */}
      <section className="mb-8">
        <h2 className="text-xl font-bold text-gray-900 mb-3 flex items-center gap-2">🎓 Education</h2>
        <div className="flex flex-col gap-4">
          {data.education.map((edu, i) => (
            <div key={i} className="bg-white border-2 border-gray-400 rounded-lg p-4 flex flex-col gap-1">
              <div className="font-bold text-gray-900">{edu.degree}</div>
              <div className="text-gray-700 text-sm">@ {edu.school}</div>
              <div className="text-gray-600 text-xs">{edu.startDate.toLocaleDateString()} - {edu.endDate ? edu.endDate.toLocaleDateString() : 'Present'}</div>
              <div className="text-gray-800 text-xs">{edu.description}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-gray-50 border-2 border-gray-400 rounded-xl p-4 flex flex-col gap-2">
        ...
        <div className="mt-2">
          ...
          <div className="text-gray-700 text-xs">Phone: {data.personalInfo.phone}</div>
          <div className="text-gray-700 text-xs">Location: {data.personalInfo.location}</div>
        </div>
      </section>
    </div>
  )
...
export const MinimalCV_EXPORT_SIZE = { width: 1200, height: 1700 } 